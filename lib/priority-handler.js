...
//format: dl/md
function getPriority(nameid, format) {
  try {
    if (jsonResponseDl) {
    }
  } catch (err) {
    const dbdl = document.createElement("script");
    dbdl.src = "lib/carddb-dl.js";
    document.head.appendChild(dbdl);
  }
  if (format == "md") {
    try {
      if (jsonResponseDl) {
      ...
      const dbmd = document.createElement("script");
      dbmd.src = `lib/carddb-md.js`;
      document.head.appendChild(dbmd);
    }
  }
  if (format == "dl") {
    const cardObj = isNaN(nameid)
      ? jsonResponseDl.filter((a) => a.name == nameid)[0]
      : jsonResponseDl.filter((a) => a.konamiID == Number(nameid))[0];
    return dlCardPriority(
      cardObj.konamiID ? cardObj.konamiID : "0000000000",
      cardObj.monsterType,
      cardObj.type,
      cardObj.rarity,
      cardObj.level,
  ...
  if (format == "md") {
    const cardObj = isNaN(nameid)
      ? jsonResponseDl.filter((a) => a.name == nameid)[0]
      : jsonResponseMd.filter((a) => a.konamiID == Number(nameid))[0];
    return mdCardPriority(
      cardObj.konamiID ? cardObj.konamiID : "0000000000",
      cardObj.monsterType,
      cardObj.type,
      cardObj.level,
