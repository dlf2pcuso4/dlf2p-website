...
const paramDeckcode = params.get("deckcode");
var jsonDataDl, jsonDataDlFiltered, jsonDataMd, jsonDataMdFiltered;
var deckcodeArray = [];
var deckArray = [];
...
window.onload = async function () {
  ...
  //let jsonResponseMd = await fetch(
  //  "https://dlf2pcuso4.github.io/ygo-deck-builder/lib/carddb-md.json"
  //);
  //jsonDataMd = await jsonResponseMd.json();
...
function loadDeck() {
  if (btnLoad.innerHTML == "Load" && loaddeck.value) {
    ...
    deckcodeArray = decodeDeck(loaddeck.value).match(/.{1,10}/g);
...
function download() {
  if (btnDownload.innerHTML == "Download Deck Image") {
    ...
    cnvTemp.height = cnvDeck.height + cnvExDeck.height + 20;
    document.body.appendChild(cnvTemp);
    ctxTemp.drawImage(cnvDeck, 0, 0);
    ctxTemp.drawImage(cnvExDeck, 0, cnvDeck.height + 20);
