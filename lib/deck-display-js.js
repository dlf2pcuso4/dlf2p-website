...
const paramDeckcode1 = params.get("d1");
var jsonDataDl,
  jsonDataDlFiltered,
  jsonDataMd,
  jsonDataMdFiltered,
...
function loadDeck() {
  if (btnLoad.innerHTML == "Load" && loaddeck.value) {
    ...
    if (paramDeckcode1) {
      console.log("yes");
      const deckcodeJson = decodeDeckcode1(paramDeckcode1);
      console.log({ deckcodeJson });
      document.getElementById("deckSkill").innerHTML = deckcodeJson.skill;
      document.getElementsByTagName("title")[0].innerHTML = deckcodeJson.title;
      document.getElementsByTagName("h1")[0].innerHTML = deckcodeJson.title;
      deckArray = deckcodeJson.deckArray;
    } else {
      //set skill
