...
window.onload = async function () {
  ...
  for (let i = 0; i < jsonResponseFilters.length; i++) {
    ...
    filter.value =
      jsonResponseFilters[i].property + "_" + jsonResponseFilters[i].value;
...
function addFilter() {
  if (searchfilter.value != "none_") {
    document.getElementById("filterlist").innerHTML +=
      ...
      jsonResponseFilters.filter(
        (a) => a.property + "_" + a.value == searchfilter.value
      )[0].name +
      "</div>";
  }
}
...
function searchCard() {
  if (btnSearch.innerHTML == "Search") {
    ...
    for (let i = 0; i < appliedFilters.length; i++) {
      ...
      if (searchfilter.value.split("_")[0] == "monsterType") {
