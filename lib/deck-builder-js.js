...
const swfilter = document.getElementById("swfilter");
const swfilterbox = document.getElementById("swfilterbox");
...
window.onload = async function () {
  ...
  if (paramDeckcode) {
    document.getElementById("deckname").value = paramDeckcode.split("_")[2];
  ...
  if (btnSearch.innerHTML == "Search") {
    ...
    if (searchfrom.value == "dl") {
      cardData = jsonDataDl.filter(function (a) {
        ...
        if (e == "deck") {
          if (deckArray.filter((b) => b.id == a.konamiID).length) {
            return (
    ...
    //filter rush cards
    if (document.getElementById("showrush").checked) {
      cardData = cardData.filter((a) => a.rush);
    } else {
      cardData = cardData.filter((a) => !a.rush);
    }
    //filter small world bridges
    if (swfilterbox.children.length == 2) {
      let nameid1 = swfilterbox.children[0].id.replace("swfilterimg", "");
      let nameid2 = swfilterbox.children[1].id.replace("swfilterimg", "");
      let card1 = jsonDataDl.filter(
        (a) => a.konamiID == nameid1 || a.name == nameid1
      )[0];
      let card2 = jsonDataDl.filter(
        (a) => a.konamiID == nameid2 || a.name == nameid2
      )[0];
      cardData = cardData.filter((a) => {
        let match1 = 0;
        let match2 = 0;
        if (card1.race == a.race) {
          match1++;
        }
        if (card1.attribute == a.attribute) {
          match1++;
        }
        if (card1.level == a.level) {
          match1++;
        }
        if (card1.atk == a.atk) {
          match1++;
        ...
        if (card2.race == a.race) {
          match2++;
        }
        if (card2.attribute == a.attribute) {
          match2++;
        }
        if (card2.level == a.level) {
          match2++;
        }
        if (card2.atk == a.atk) {
          match2++;
    ...
    for (let c = 0; c < imgCount; c++) {
      document.getElementById("img" + c).onload = function () {
        ...
        ctxResults.drawImage(
          document.getElementById("img" + c),
          (421 * cardSize + 10) * (c % imgPerRow),
          688 * cardSize * Math.floor(c / imgPerRow) + 74 * cardSize,
          421 * cardSize,
          614 * cardSize
        );
        if (cardData[c].rarity) {
          ctxResults.drawImage(
            document.getElementById(
              searchfrom.value + "tag" + cardData[c].rarity
            ),
            (421 * cardSize + 10) * (c % imgPerRow) + 205 * cardSize,
            688 * cardSize * Math.floor(c / imgPerRow) + 20 * cardSize,
            216 * cardSize,
            54 * cardSize
          );
        }
        if (cardData[c].banStatus) {
          ctxResults.drawImage(
            document.getElementById(cardData[c].banStatus),
            (421 * cardSize + 10) * (c % imgPerRow),
            688 * cardSize * Math.floor(c / imgPerRow) + 74 * cardSize,
            110 * cardSize,
            110 * cardSize
...
function selectCard(e) {
  if (autoadd.checked || swfilter.checked) {
    addCard(e);
  } else {
...
function addCard(e) {
  ...
  if (swfilter.checked) {
    ...
    if (
      ...
      refreshDeck();
      deckIndex++;
      document.getElementById("notificationImg").src = document.getElementById(
        "img" + cardSelected
      ).src;
...
//add small world filter
function addSwFilter() {
  if (swfilterbox.children.length < 2) {
    if (
      ...
      let nameid = cardData[cardSelected].konamiID
        ? cardData[cardSelected].konamiID
        : cardData[cardSelected].name;
      let img = document.createElement("img");
      img.id = "swfilterimg" + nameid;
      img.onerror = function () {
        this.onerror = null;
        this.src = "images/noimage.jpg";
      };
...
function importDeck() {
  if (btnImport.innerHTML == "Import") {
    ...
    clearDeck();
    //set skill
    let dsSplit = convertToV2(
      document.getElementById("importdeck").value
    ).split("_");
    ...
    if (dsSplit[2]) {
      document.getElementById("deckname").value = decodeURIComponent(
        dsSplit[2]
      );
    }
...
function copyDeck() {
  if (btnCopy.innerHTML == "Copy Deck Code") {
    ...
    let link =
      ...
      "_" +
      encodeURIComponent(document.getElementById("deckname").value);
    if (specialCards) {
