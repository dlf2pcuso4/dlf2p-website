...
function searchCard() {
  if (btnSearch.innerHTML == "Search") {
    ...
    if (swfilterbox.children.length) {
      let nameid1 = swfilterbox.children[0].id.replace("swfilterimg", "");
      let nameid2 =
        swfilterbox.children.length == 1
          ? swfilterbox.children[0].id.replace("swfilterimg", "")
          : swfilterbox.children[1].id.replace("swfilterimg", "");
      let card1 = cardData.filter(
        (a) => a.konamiID == nameid1 || a.name == nameid1
      )[0];
      let card2 = cardData.filter(
