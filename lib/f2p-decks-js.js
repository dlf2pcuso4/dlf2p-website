...
function refreshMenu(change) {
  ...
  switch (change) {
    case "category":
      const categories = ["2022", "2023", "2024", "2025", "All"];
...
function loaddeck(url, format) {
  ...
  for (card of deckcodeArr) {
    ...
    cardimg.onerror = function () {
      this.onerror = null;
      this.src = `https://s3.duellinksmeta.com/cards/${
        jsonResponseDl.filter(
          (a) =>
            Number(a.konamiID) ==
            Number(this.src.split("images/cards/")[1].split(".jpg")[0])
        )[0]._id
      }_w360.webp`;
    };
...
function popup(card) {
  ...
  document.getElementById("cardinfoImg").onerror = function () {
    this.onerror = null;
    this.src = `https://s3.duellinksmeta.com/cards/${
      jsonResponseDl.filter(
        (a) =>
...
function testHands(url) {
  if (!document.querySelector("ygol")) {
    //check if rush
    let firstCardID = document
      .getElementById("maindeck")
      .firstChild.src.split("cards/")[1]
      .split(".jpg")[0];
    let isRush = !isNaN(firstCardID)
      ? jsonResponseDl.filter((a) => a.konamiID == firstCardID)[0].rush
      : jsonResponseDl.filter((a) => a.name == firstCardID)[0].rush;
    //create ygol window
    document.body.insertAdjacentHTML(
      ...
      `<div id='ygol-popup' class="flexdivc" style="flex-direction:column;position:fixed;width:100%;height:100%;inset:0px;align-items:center;background-color:#000000C0;z-index:90;cursor:pointer;">
        <div id="ygol-inner-popup" style="width:1280px;height:720px;background-color:#000000">
          <ygol dlf2pV2="${url.split("d=")[1]}" ${
        isRush ? 'rush="true"' : ""
      }></ygol>
        </div>
      </div>`
    );
    document.getElementById("ygol-inner-popup").onmousedown = (event) => {
      document
        .getElementById("ygol-inner-popup")
        .setAttribute("clicked", "true");
    };
    document.getElementById("ygol-popup").onmouseup = (event) => {
      if (
        document.getElementById("ygol-inner-popup").getAttribute("clicked") ==
        "false"
      )
    ...
    document.getElementById("ygol-popup").style.display = "flex";
    if (
      ...
      document
        .querySelector("ygol")
        .setAttribute("dlf2pV2", url.split("d=")[1]);
    }
  }
}
