...
function testHands(url) {
  if (!document.getElementById("ygol-mainDiv")) {
    document.body.insertAdjacentHTML(
      "afterbegin",
      `<div id='ygol-popup' class="flexdivc" style="flex-direction:column;position:fixed;width:100%;height:100%;inset:0px;align-items:center;background-color:#000000C0;z-index:90;cursor:pointer;"><div class="flexdivc"><div id="ygol-mainDiv" style="width:1280px;height:720px;background-color:#000000"></div></div></div>`
    );
    document.getElementById("ygol-mainDiv").onmousedown = (event) => {
      document.getElementById("ygol-mainDiv").classList = "clicked";
    };
    document.getElementById("ygol-popup").onmouseup = (event) => {
      if (document.getElementById("ygol-mainDiv").classList != "clicked")
        document.getElementById("ygol-popup").style.display = "none";
      document.getElementById("ygol-mainDiv").classList = "";
    };
    ...
    const l = () => {
      try {
        if (allowAutoLoadDeck) {
          autoLoadDeck(convertUrlToYdk(url));
        }
      } catch (e) {
        setTimeout(() => {
          l();
