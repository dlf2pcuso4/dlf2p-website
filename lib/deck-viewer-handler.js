...
function newDeckElement(url, format, w, h, cw) {
  ...
  for (let card of deckcodeArr) {
    let cardimgHTML = `<img src="images/cards/${
      ...
      isNaN(card) ? card : Number(card)
    }','${format}')" onerror="deckViewerSwitchSrc(this)">`;
    ...
    if (cardObj.banStatus && format != "tcg")
...
function deckViewerSwitchSrc(img) {
  img.onerror = null;
  img.src = `https://s3.duellinksmeta.com/cards/${
    jsonResponseDl.filter(
      (a) =>
        Number(a.konamiID) ==
        Number(img.src.split("images/cards/")[1].split(".jpg")[0])
    )[0]._id
  }_w360.webp`;
}
